...
class CharacteristicCallback: public BLECharacteristicCallbacks {
    // Convert byte array to unsigned char array
    ...
    void sendData(String inp) { 
        ...
        for (int i = 0; i < len; i += chunkSize) {
            ...
            pCharText->notify();
            delay(100); // Delay to avoid congestion in the Bluetooth stack
        }
    }
...
//            byteArrayToUnsignedCharArray(downlinkData, totalUnsigned, datalength);
        ...
        if(downlinkData[0] == 0) { // If no downlink data, read from file
            ...
            for(int attempts = 0; attempts < totalMessages; attempts++) { // For every message store data in fullChain
                ...
                if(state == 0 && memcmp(prevDownlink, downlinkData, downlinkSize) != 0) { //Memcmp to make sure the downlink isn't received twice
...
void setup() {
    ...
    st7735.st7735_write_str(0, 10, "System start, initializing Radio...", Font_11x18, ST7735_RED, ST7735_BLACK);
    ...
    node.setCSMA(6, 2, true); // Enable CSMA
    ...
    // Initialize BLE Server and Service
    ...
    USBSerial.println("Waiting for a client connection to notify...");
...
void loop() {
    if (deviceConnected) {
        // Do nothing if device is connected
    }

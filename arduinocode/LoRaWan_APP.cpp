...
bool SendFrame( void )
{
	...
	else
	{
		...
		else
		{
			...
			printf("Sending confirmed uplink frame:\n");
			printf("Type: MCPS_CONFIRMED\n");
			printf("Port: %d\n", appPort);
			printf("Data: ");
...
static void OnTxNextPacketTimerEvent( void )
{
	...
	if( status == LORAMAC_STATUS_OK )
	{
		...
		{
			printf("network joined so sending data!\r\n");
		...
		else
		{
			...
			{
				printf("TImerEvent sleep");
				deviceState = DEVICE_STATE_SLEEP;
			}
			else
			{
				printf("TImerEvent cycle");
...
void LoRaWanClass::init(DeviceClass_t lorawanClass,LoRaMacRegion_t region)
{
	...
	printf("TxNextPacketTimer address: %p\n", (void *)&TxNextPacketTimer);
  ...
  {
	printf("SENDING STATE ACTIVATED");
    deviceState = DEVICE_STATE_SEND;
  }
}
...
void LoRaWanClass::join()
{
	if( overTheAirActivation )
	{
		...
		{
			joinedNetwork = true;
			printf("TImerEvent sleep->cycle cuz ok");
